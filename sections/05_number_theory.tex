...
\textbf{Description:} Solve system of congruences $x \equiv a_1 \pmod{m_1}$, $x \equiv a_2 \pmod{m_2}$, ... Time: O(n log M) where M is product of moduli.
...
def chinese_remainder(remainders, moduli):
    ...
    for r, m in zip(remainders, moduli):
        p = prod // m
        g, inv, _ = extgcd(p, m)
        # inv may be negative, normalize it
        inv = (inv % m + m) % m
        total += r * inv * p
...
\textbf{Description:} $\phi(n)$ = count of numbers $\leq n$ coprime to n. Time: O($\sqrt{n}$).
