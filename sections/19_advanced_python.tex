...
# Fast I/O: see the dedicated Input/Output section for recommended setup using
# `input = sys.stdin.readline` and other fast I/O patterns (Section 01_io).
# Avoid redefining `input`/`print` repeatedly across sections; set them once
# in your submission template when needed.
...
\subsubsection*{ASCII conversions (quick reference)}
\begin{center}
\small
\begin{tabular}{@{}ll|ll@{}}
\textbf{Char} & \textbf{Code} & \textbf{Char} & \textbf{Code} \\
\hline
'A' & 65 & 'a' & 97 \\
'Z' & 90 & 'z' & 122 \\
'0' & 48 & '9' & 57 \\
' ' (space) & 32 & '\textbackslash n' & 10 \\
'!' & 33 & '?' & 63 \\
\end{tabular}
\end{center}

\medskip
Use \texttt{ord(char)} to get the numeric code and \texttt{chr(code)} to convert back.
